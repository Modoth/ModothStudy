...
export class ScriptAppService {
  getNodeApp = (context: { appTag: string, apps: Map<string, string> },
    tags: NodeTag[], nodesApi: NodesApi, configsService: ConfigsService) => (): Observable<string | {}> => {
      return this.getScriptType(tags, configsService).pipe(
        switchMap(appType => iif(
          () => !appType,
          null,
          iif(
            ...
            iif(
              ...
              this.getAppTag(configsService).pipe(
                switchMap(tag => this.getAppContent(tag as string, appType, nodesApi, context))
              )
            )
          )
        )
  ...
  getAppContent = (tagName: string, type: string, nodesApi: NodesApi, context: { appTag: string, apps: Map<string, string> }): Observable<string> => {
    if (!tagName) return empty()
    return nodesApi.getBlogsByTag(tagName, type, 1).pipe(
      map(apiRes => apiRes ? apiRes.data.data[0] && apiRes.data.data[0].content : null),
  ...
  getScriptType = (tags: NodeTag[], configsService: ConfigsService): Observable<string> => {
    return configsService.getConfig(Configs.AppConfigsEnum.SCRIPTTAG.toString()).pipe(
