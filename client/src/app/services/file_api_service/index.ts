...
class FileApiService {
    ...
    public fetch (urlObj: {url: string, method: Enum_Request_Method}, params: {blob: Blob, serch?: object}): Observable<ApiResultString> {
      let {url, method} = urlObj
      let {blob, serch} = params
      const fd = new FormData();
      fd.append('file', blob);
      return from(fetch(this._getFetchUrl(url, serch), {
        method,
        body: fd
      })).pipe(
