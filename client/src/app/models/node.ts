...
function updateNode(
  node: Node,
  [docTypeTag, solutionTypeTag, wechatShareTag, autoPlayTag, sandboxAppTag]: [
    string,
    string,
    string,
    string,
  ...
  node.tagDict = node.tags
    ? new Map(node.tags.map((t) => [t.name, t] as any))
    : new Map();
  var realTagDict = node.tagDict;
  if (node.reference) {
    realTagDict = node.reference.tags
      ? new Map(node.reference.tags.map((t) => [t.name, t] as any))
      : new Map();
  ...
  const solutionType =
    realTagDict.get(solutionTypeTag) || node.tagDict.get(solutionTypeTag);
  ...
  const autoPlay =
    realTagDict.get(autoPlayTag) || node.tagDict.get(autoPlayTag);
  node.autoPlay = !!autoPlay;
  const sandBoxApp =
    realTagDict.get(sandboxAppTag) || node.tagDict.get(sandboxAppTag);
  node.sandBox = !!sandBoxApp;
}

export function ConvertToNodeModel(
  nodes: NodeItem[],
