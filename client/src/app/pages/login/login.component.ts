...
import { LoginUser, Configs, LoginService, ApiResultString } from 'src/app/apis';
...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  public readonly pwdPopupConfig = [
    { label: Configs.UiLangsEnum.Password.toString(), type: 'password', key: 'oldPwd' },
    { label: Configs.UiLangsEnum.NewPassword.toString(), type: 'password', key: 'newPwd' },
    { label: Configs.UiLangsEnum.NewPassword.toString(), type: 'password', key: 'rePwd' }
  ]
  ...
  login = () => {
    ...
    this.isLoading = true;
    this.pwd = '';
    this.appService.login(name, pwd).subscribe(apiRes => {
      this.isLoading = false;
      this.appService.navagateBackOrHome();
    }, error => { this.isLoading = false; this.notofyService.toast(error) });
  }
  ...
  logout = () => {
    this.appService.logout().subscribe({
      error(er) { this.notofyService.toast(er) }
    })
  }
  ...
  changeNickNameSave(data) {
    ...
    this.loginApi.updateNickName(nickName.trim()).pipe(
      tap(apiRes => {
  ...
  changeAvatar(data) {
    ...
    this.fileApiService.fetch(fileApiUrls.Login_UpdateAvatar, { blob: data }).pipe(
      tap(apiRes => {
        if (!apiRes.result) throw apiRes.error;
  ...
  changePwd(data: { oldPwd, newPwd, rePwd }) {
    this.configService.getConfig(Configs.AppConfigsEnum.PWDEXP).subscribe(reg => {
      if (!reg) {
      ...
      this.loginApi.updatePwd(newInfo).pipe(
        tap(apiRes => {
          if (!apiRes.result) throw apiRes.error;
  ...
  ngOnInit() {
    ...
    this.subscriptions.push(this.appService.explorerHistory.subscribe(his => this.explorerHistory = his))
  }
