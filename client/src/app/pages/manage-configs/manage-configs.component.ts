...
import { ConfigsService as ConfigsApi, ConfigItem, Configs, ApiResultString } from 'src/app/apis';
...
export class ManageConfigsComponent implements OnDestroy {
  ...
  saveImage(data) {
    ...
    this.fileApiService.fetch(fileApiUrls.Configs_UpdateImageValue, { blob: data, serch: { id: this.editData.id } }).pipe(
      tap(apiRes => {
        if (!apiRes.result) throw apiRes.error;
        this.editData.value = apiRes.data
      }),
      switchMap(apiRes => this.configsService.setConfig(this.editData.key, apiRes.data))
  ...
  public updateConfig(config: ConfigItem, value: string) {
    this.configsApi.updateValue(config.id, JSON.stringify(value)).pipe(
      tap(apiRes => {
        if (!apiRes.result) throw apiRes.error;
        config.value = value
      }),
  ...
  public resetConfig = (config: ConfigItem) => {
    this.configsApi.resetValue(config.id).pipe(
      tap(apiRes => {
        if (!apiRes.result) throw apiRes.error;
        config.value = null
      }),
      switchMap(() => this.configsService.setConfig(config.key, config.defaultValue))
    ).subscribe({ error: er => this.notifyService.toast(er) })
  }
  ...
  public reloadConfigs = ({ filter, pageId, pageSize }) => {
    this.configsApi.allConfigs(filter, +pageId * pageSize, pageSize).pipe(
      tap(apiRes => {
        if (!apiRes.result) throw apiRes.error;
