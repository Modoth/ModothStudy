...
import { NodeItem, NodesService, LoginUser, TagItem, Configs } from 'src/app/apis';
...
export default class NodeBase implements OnDestroy, OnInit {
  ...
  constructor(public _appService: AppService,
    ...
    public _nodesApi: NodesService, public _configsService: ConfigsService, ) {

  }

  public updateShared = (shared: boolean) => {
    this._nodesApi.updateNodeShared(this.nodeId, shared).pipe(
      tap(apiRes => {
  ...
  public addTag({ tag, value }: { tag: TagItem, value: string }) {
    ...
    this._nodesApi.updateTag(this.currentNode.id, tag.id, value).pipe(
      tap(apiRes => {
  ...
  public removeTag(tag) {
    ...
    this._nodesApi.removeTag(this.currentNode.id, tag.id).pipe(
      tap(apiRes => {
  ...
  ngOnInit() {
    this.subscriptions.push(this._appService.loginUsers.subscribe(async (user) => {
      this.loginUser = user;
