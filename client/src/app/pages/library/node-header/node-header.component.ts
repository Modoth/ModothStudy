...
export class NodeHeaderComponent {
  ...
  public get backPath() {
    return `/library/list/${
      (this.parentNode && this.parentNode.id) || "0"
    }/ /0`;
  }
  ...
  public selectedChanged(selected: NodeItem) {
    ...
    this.canMoveTo =
      selected != null &&
      selected.type == NodeItem.TypeEnum.Folder &&
      (this.parentNode ? this.parentNode.id != selected.id : true) &&
      (excludePathPrefix == null ||
        !selected.path.startsWith(excludePathPrefix));
  }
  ...
  public moveFloder() {
    ...
    this.nodesApi
      .move(this.currentNode.id, this.selectFloderId)
      .pipe(
        tap((apiRes) => {
  ...
  public async rename(data: { name }) {
    ...
    this.nodesApi
      .rename(this.currentNode.id, newNodeName)
      .pipe(
        tap((apiRes) => {
          if (!apiRes.result) throw apiRes.error;
        })
