...
import { NodeItem, NodesService, Configs, TagItem, LoginUser } from 'src/app/apis';
...
export class NodeListComponent implements OnInit, OnDestroy {
  ...
  public get canShareWx() {
    return this.wxEnabled && this.loginUser && this.loginUser.permissions && this.loginUser.permissions[Configs.PermissionDescriptionsEnum.THIRDSHARE]
  }
  ...
  public updateWxShared = (shared: boolean) => {
    this.nodesApi.updateWxShare(this.nodeId, shared).pipe(
      tap(apiRes => {
        if (!apiRes.result) throw apiRes.error;
        let tag = apiRes.data;
  ...
  ngOnInit() {
    this.subscriptions.push(this.appService.loginUsers.subscribe(async (user) => {
      ...
      this.configsService.getConfig(Configs.AppConfigsEnum.WXENABLED.toString()).subscribe(enabled => {
        this.wxEnabled = !!enabled;
      }))
  }

  public readonly nodeTypes = [NodeItem.TypeEnum.Blog, NodeItem.TypeEnum.Reference, NodeItem.TypeEnum.Folder]
  ...
  public search = (data: { filter: string }) => {
    ...
    this.router.navigateByUrl(`/library/list/${this.nodeId || '0'}/${encodeURI(data.filter.trim())}/0`);
  }
  ...
  public addRef = () => {
    ...
    this.nodesApi.createRefNode(this.newRefName, this.currentNode.id, this.selectedRef.id)
      .pipe(
        tap(apiRes => {
          if (!apiRes.result) throw apiRes.error;
  ...
  delete(data: { name }) {
    ...
    if (name == this.deleteData.name) {
      this.nodesApi.removeNode(this.deleteData.id).pipe(
        tap(apiRes => {
          if (!apiRes.result) throw apiRes.error;
  ...
  public createNode(name: string, type) {
    this.nodesApi.createNode(name, type, this.nodeId).pipe(
      tap(apiRes => {
        if (!apiRes.result) {
          throw apiRes.error;
        }
  ...
  public getSubNodes(filter, pageId, pageSize) {
    var nodeId = this.currentNode && this.currentNode.reference ? this.currentNode.reference.id : this.currentNode && this.currentNode.id;
    this.nodesApi.subNodesOrFilterAllSubNodes(nodeId, filter, +pageId * pageSize, pageSize).pipe(
      tap(apiRes => {
        if (!apiRes.result) {
          this.subNodes = [];
          this.totalCount = 0;
      ...
      tap(nodes => {
        ...
        if (!filter && this.loginUser && this.subNodes.length > 0) {
          let myRootNode = null
          if (!nodeId) {
            let myRootIdx = this.subNodes.findIndex(n => n.user && n.user.id === this.loginUser.id);
            if (myRootIdx >= 0) {
            ...
            let mySolutionsIdx = this.subNodes.findIndex(n => n.path == "/!solutions");
            if (mySolutionsIdx >= 0) {
  ...
  private getPath = (node: NodeItem) => {
    ...
    // TODO: 
    return isBlog ?
      (this.isOwnNode ? `/library/detail/${node.id || '0'}`
