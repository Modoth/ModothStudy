...
export class QueryComponent implements OnInit, OnDestroy {
  ...
  public getRouteLink(page = null, filter = null) {
    ...
    return this.router.createUrlTree([this.menu.name, filter || " ", page], { relativeTo: this.route.parent }).toString();
  }

  public reloadNodesOnPage() {
    this.nodesApi.queryNodes(this.menu.query, this.filter, this.currentPage * this.pageSize, this.pageSize).pipe(
      tap(apiRes => {
        if (!apiRes.result) {
          this.nodes = [];
          this.totalCount = 0;
