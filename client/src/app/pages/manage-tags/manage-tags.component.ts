...
export class ManageTagsComponent implements OnDestroy {
  ...
  async saveText(data: { values }) {
    ...
    this.tagsApi
      .updateTagValues(this.editTagData.id, values)
      .pipe(
        tap((apiRes) => {
  ...
  public readonly tagTypes = [
    TagItem.TypeEnum.Bool,
    TagItem.TypeEnum.Number,
    TagItem.TypeEnum.Enum,
    TagItem.TypeEnum.String,
    TagItem.TypeEnum.Url,
    TagItem.TypeEnum.Private,
  ];
  ...
  public reloadTags = ({ filter, pageId, pageSize }) => {
    this.tagsApi
      .allTags(this.filter, +pageId * pageSize, pageSize)
      .pipe(
        tap((apiRes) => {
          if (!apiRes.result) throw apiRes.error;
