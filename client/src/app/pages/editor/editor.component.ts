...
export class EditorComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    const mySub = this.appService.loginUsers
      .pipe(
        distinctUntilChanged(),
        tap((user) => {
          this.loginUser = user;
          this.hasReplySolutionPersission =
            this.loginUser != null &&
            this.loginUser.permissions != null &&
            this.loginUser.permissions[
              Configs.PermissionDescriptionsEnum.REPLYSOLUTION
            ];
        }),
  ...
  public openLocation = () => {
    if (this.parentNode) {
      ...
      var path = `/library/list/${
        node.type == NodeItem.TypeEnum.Folder ? node.id : node.id
      }/ /0`;
  ...
  public updateMySolution(solution: string, item: NodeViewItem) {
    ...
    this.subscriptions.push(
      this.configsService
        ...
        .pipe(
          switchMap((solutionPrefix) =>
            this.nodesApi.updateBlogSolution(
              item.node.id,
              solutionPrefix + item.node.name,
  ...
  public loadSolution = (item: NodeViewItem) => {
    ...
    this.nodesApi
      .getBlogDefaultSolution(item.node.id)
      .pipe(
        switchMap((apiRes) => {
          if (!apiRes.result) {
            this.notifyService.toast(apiRes.error);
  ...
  public update() {
    this.nodesApi
      .allLevelNodes(this.nodeId, 0, this.MAX_COUNT)
      .pipe(
        tap((apiRes) => {
          if (!apiRes.result) throw apiRes.error;
          this.nodes = [];
        ...
        switchMap((nodes) =>
          from(nodes || []).pipe(withLatestFrom(of(new NodeViewItem(nodes[0]))))
        ),
        map(([node, rootNode]) => {
          ...
          n.ownNode =
            this.loginUser != null && n.node.user.id == this.loginUser.id;
          n.pythonService =
            n.node.solutionType === "python"
        ...
        tap((nodes) => {
          nodes.length > 0 &&
            this.appService.putExplorerHistoryItem({
              title: nodes[0].node.name,
              id: nodes[0].node.id,
              date: new Date(),
            });
          this.nodes = nodes;
    ...
    this.nodesApi
      ...
      .pipe(
        tap((apiRes) => {
          ...
          this.parentNode =
            apiRes.data[apiRes.data.length - 2] ||
            apiRes.data[apiRes.data.length - 1];
        })
      )
      .subscribe({ error: (er) => this.notifyService.toast(er) });
  }
  ...
  public getStyle(): Observable<string[]> {
    return forkJoin(
      this.configsService.getConfig(
        Configs.AppConfigsEnum.EDITORSTYLETAG.toString()
      ),
      this.configsService.getConfig(
        Configs.AppConfigsEnum.EDITORSTYLES.toString()
      )
    ).pipe(
