...
import { CommentService, LoginUser, Configs, CommentItem, NodeItem } from 'src/app/apis';
...
export class CommentsViewerComponent implements OnInit, OnDestroy {
  ...
  constructor(public commentService: CommentService, public appService: AppService,
    public notifyService: NotifyService) {
  }

  ngOnInit() {
    this.subscriptions.push(this.appService.loginUsers.subscribe(user => this.loadComments(user)));
  }

  loadMoreComments() {
    let sub = this.commentService.getBlogComments(this.node.id, (this.currentPage + 1) * this.pageSize, this.pageSize).pipe(
      tap(apiRes => {
  ...
  loadComments(user: LoginUser) {
    this.loginUser = user;
    this.hasCommentPermission = user != null && user.permissions != null &&
  ...
  addComment() {
    ...
    let sub = this.commentService.addComment(this.node.id, comment).pipe(
      tap(apiRes => {
        if (!apiRes.result) throw apiRes.error;
      })
    ).subscribe(apiRes => {
      const lastCommentsList = this.commentsList[this.commentsList.length - 1];
      if (lastCommentsList && lastCommentsList.length < this.pageSize) {
        const c = {
          id: apiRes.data, userName: this.loginUser.nickName,
          userId: this.loginUser.id, userAvatar: this.loginUser.avatar,
          detail: comment
        };
  ...
  deleteComment(comment: CommentItem, comments: CommentItem[]) {
    let sub = this.commentService.deleteComment(comment.id).pipe(
      tap(apiRes => {
        if (!apiRes.result) throw apiRes.error;
      })
    ).subscribe(apiRes => {
