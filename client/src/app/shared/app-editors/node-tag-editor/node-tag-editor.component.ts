...
export class NodeTagEditorComponent implements OnChanges {
  ...
  @Output() tagAdded: EventEmitter<{ tag: TagItem, value: string }> = new EventEmitter();
  ...
  constructor(public tagsApi: TagsService, public notifyService: NotifyService) {
  }
  ...
  loadTags(): Observable<any> {
    ...
    return this.tagsApi.allTags().pipe(
      tap(apiRes => {
        ...
        this.allTags = apiRes.data.data.reduce((m, tag) => m.set(tag.id, { tag, added: false }), new Map());
        this.tags = Array.from(this.allTags.values());
      })
    )
  }
  ...
  public get enumList() {
    ...
    return this.editTag.values.split(' ').map(v => v.trim()).filter(v => v !== '')
  }
