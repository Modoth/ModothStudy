import { Component, Input, Output, EventEmitter, OnDestroy, ViewChild, TemplateRef } from '@angular/core';
...
export class PopupComponent implements OnDestroy {
  ...
  show = () => {
    ...
    if (this.type !== PopupType.page || !this.customizeBtnGroupRef) {
      this.afterClosedSubscription = this.dialogRef.afterClosed().subscribe(isSave => {
        if (isSave) {
          this.saved.emit()
        } else {
          this.canceled.emit()
