...
namespace ArticlesImporter.Converts
{
    public class FormulaConverter
    {
        public string Convert(string fIn)
        {
            ConvertEquation ce = new ConvertEquation();
            if (ce.Convert(new EquationInputFileWMF(fIn),
                       new EquationOutputClipboardText("Texvc.tdl")))
            {
                var latex = Clipboard.GetText();
                if(latex != null)
                {
                    latex = latex.Replace("\r\n<math>", "");
